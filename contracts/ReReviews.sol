...
contract ReReviews is Owned {
  ...
  function getNumberOfReviews() public view returns (uint) {
    return reviewCounter;
  }

  // fetch and returns all reviews IDs available for visitor
  function getReviewsToView() public view returns (uint[] memory) {
  ...
  function viewReview(uint _id,uint _totalUp, uint _totalDown) payable public {
    ...
    // trigger the event
    emit viewReviewEvent(_id, review.reviewer, review.visitor, review.name, review.totalUpvotes, review.totalRewards, review.totalDownvotes, review.ipfsHash);
  }
